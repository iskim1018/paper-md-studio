# Changelog

이 파일은 [Keep a Changelog](https://keepachangelog.com/ko/1.0.0/) 형식을 따릅니다.

## [0.6.0] - 2026-08-17

### 추가

- **엑셀(.xlsx, .xls) 변환 지원** — 날짜·통화·백분율 같은 표시형식이 엑셀에서
  보이던 그대로 나오고(`45000`이 아니라 `2026-08-17`), 병합 셀·시트 안
  이미지·하이퍼링크도 함께 옮겨진다. `.xls`와 `.xlsx`는 같은 내용이면 같은
  결과가 나온다 — 확장자 때문에 결과가 갈리지 않는다
- **엑셀 원본 뷰어** — 변환 결과와 원본을 나란히 대조할 수 있다. 숨겨져
  있던 행·열은 지우지 않고 빗금·흐림으로 표시해 무엇이 빠졌는지 눈으로
  확인할 수 있다. 시트가 여러 개면 이동 탭이 뜬다
- **숨긴 시트·행·열을 포함할지 고를 수 있다** — 기본은 제외하되 무엇이 몇 개
  빠졌는지 결과 배너로 알리고, 배너의 버튼 한 번으로 포함해 다시 변환한다
  (CLI는 `--include-hidden`). 숨김의 의도가 대외비인지 단순히 접어둔 것인지는
  문서만 봐서 알 수 없으므로 사용자가 정한다
- **구버전 한글 문서(HWP 3.x)와 HWPML 지원** — 확장자가 같은 `.hwp`라도 내부
  형식을 자동으로 판별한다

### 변경

- **PDF의 표가 이제 표로 나온다** — 변환 엔진을 교체했다. 종전 엔진은 표가
  많은 실물 문서에서 표를 하나도 잡아내지 못했다. 문장 읽기 순서 정확도도
  함께 올랐다 (실측 59% → 79%)
- **HWP 5.0 변환에 Java가 필요 없다** — 기본 경로가 순수 TypeScript 파서로
  바뀌었다. 기존 Java 경로는 폴백으로 남아 있다
- **모든 포맷의 표가 같은 규칙을 따른다** — HWPX·DOCX·PDF·엑셀 어디서 왔든
  병합 셀은 `←`/`↑`로 표시되고 행마다 열 수가 맞춰진다. DOCX의 병합 표가
  행 단위로 조각나며 무너지던 문제도 이 과정에서 해결됐다
- 한컴 문서의 특수문자(체크박스 ☑·□, 불릿 등)가 더 정확하게 표시된다
- REST·MCP 응답에도 변환 경고가 실린다 — 숨긴 시트가 조용히 빠지는 일이
  없도록, 캐시된 응답에서도 같은 경고를 돌려준다

### 수정

- **엑셀 변환 결과에 빈 줄이 수천 개 딸려오던 문제** — 편집 프로그램이 내용과
  무관하게 미리 깔아둔 행까지 표로 옮기고 있었다. 실물 문서에서 결과물이
  절반 이하로 줄었다 (666KB → 285KB, 표 6,164행 → 1,355행). 내용은 그대로다
- **큰 표가 있는 문서에서 미리보기가 멈추던 문제** — 1,100행짜리 표에서
  74초 동안 화면이 굳었다. 이제 1초 안에 그려진다
- 의존성 보안 취약점 20건 해소

## [0.5.4] - 2026-08-04

### 변경

- **macOS 앱이 Apple 정식 서명·공증(notarization)을 받는다** — 설치 후
  "확인되지 않은 개발자" 경고나 `xattr -cr` 우회 없이 바로 실행된다

### 수정

- **PDF의 굵은 제목이 같은 글자를 수십 번 반복하던 문제** — 한글 워드프로세서는
  굵은 글씨를 굵은 글꼴 대신 같은 텍스트를 0.1pt씩 밀어 겹쳐 그리는 방식으로
  표현한다. 이를 낱낱의 글자로 읽어들여 `제안요청서`가 23번 반복되곤 했다
- **PDF에서 `제21조`가 `제 21 조`로 벌어지던 문제** — 숫자와 한글 사이에
  없던 공백이 끼어들었다. `2천8백여종`, `24시간`, `7개월` 등도 마찬가지
- **PDF 목차가 본문 제목으로 오인식되던 문제** — 목차 줄이 전부 제목으로
  잡혀 문서 개요가 뒤엉켰다. 이제 장/절 계층을 살린 목록으로 정리되고,
  점선(`······`)도 함께 사라진다
- PDF 변환 결과에서 문단 앞뒤에 남던 불필요한 공백 제거

## [0.5.3] - 2026-07-27

### 수정

- **폴더 열기 버튼이 눌러도 아무 반응이 없던 문제** — 결과 패널의 폴더
  열기(툴바·푸터 2곳)가 실제로는 동작하지 않고 있었다. 이제 결과 `.md`
  파일이 선택된 상태로 Finder/탐색기가 열린다
- 창 오른쪽 끝 버튼의 툴팁이 화면 밖으로 잘려 내용을 읽을 수 없던 문제 —
  말풍선 전체가 화면 안에 들어오도록 위치를 다시 계산한다

### 추가

- 출력 폴더 행에 **폴더 열기 버튼** — 변환 결과가 저장되는 위치를 바로
  열어볼 수 있다. "원본 폴더" 모드에서는 선택된 파일이 있는 폴더를 연다
- 폴더를 열지 못하면 그 이유를 화면에 표시한다

### 변경

- **패널 경계선을 잡기 쉽게 개선** — 조절 가능한 손잡이임이 드러나도록
  그립 표식을 상시 노출하고, 실제로 잡히는 범위를 넓혔다 (기존 대비 5배).
  마우스를 가져가면 강조되며, 화살표 키로도 조절할 수 있다
- macOS에서 안내 문구가 "탐색기" 대신 "Finder"로 표시된다

## [0.5.2] - 2026-07-27

### 추가

- **자동 업데이트** — 새 버전이 나오면 앱 시작 시 배너로 알리고, 눌러서
  설치하면 자동으로 재시작한다. 매번 직접 내려받아 재설치할 필요가 없다.
  설치 여부는 항상 사용자가 결정하며, 업데이트 확인에 실패해도(네트워크
  단절 등) 앱 사용에는 영향이 없다

### 변경

- Windows `.msi` 배포 중단 — `.exe`(NSIS) 설치 파일 하나로 통일했다.
  같은 앱의 중복 설치 형식이라 어느 쪽을 받아야 할지 혼란스러웠고, NSIS는
  관리자 권한 없이 설치된다. 기존 `.msi` 사용자는 이번 한 번만 `.exe`로
  재설치하면 이후로는 자동 업데이트를 받는다

### 참고

- 이 버전은 한 번 직접 설치해야 한다 (v0.5.1에는 업데이터가 없다).
  v0.5.3부터 자동 업데이트가 동작한다

## [0.5.1] - 2026-07-27

### 수정

- 용량이 큰 HWP/HWPX 파일을 선택하면 앱은 물론 시스템 전체가 멈추던 문제 —
  문서를 열 때 전 페이지 SVG를 한 번에 생성·주입하던 방식을 페이지
  가상화로 교체. 이제 보이는 구간 앞뒤 2쪽만 렌더하고 멀리 떨어진
  페이지는 메모리에서 해제한다 (쪽수·스크롤 위치·페이지 점프는 그대로)
- 파일을 열 때마다 검색용 인덱스를 문서 전체에 대해 미리 만들던 문제 —
  검색어를 실제로 입력한 시점에 화면을 막지 않고 나눠서 구축하도록 변경.
  표가 많은 문서를 열 때 걸리던 정지가 사라진다

### 변경

- 로딩 표시를 회전 스피너로 교체 (HWP·DOCX·PDF·DOC 미리보기) — 무거운
  작업 중에도 애니메이션이 멈추지 않아 진행 중인지 바로 알 수 있다
- 아직 그려지지 않은 미리보기 페이지는 빈 자리 대신 스피너를 표시

## [0.5.0] - 2026-07-25

### 추가

- 도움말 매뉴얼 모달 (`F1` / `Cmd(Ctrl)+/`) — 빠른 시작, 지원 형식,
  파일 목록 조작법, 편집 모드 설명, 카테고리별 단축키 표
- 플랫폼 인식 단축키 표기 — macOS는 `⌘⇧P`, Windows는 `Ctrl+Shift+P`
  형식으로 툴팁·도움말에 일관 표시 (단일 레지스트리 공유)
- 커스텀 툴팁 — 주요 버튼에 단축키 병기, 테마 연동, 패널 경계
  클리핑 없는 포탈 렌더링
- 문서 뷰어 플로팅 페이지/줌 컨트롤 자동 숨김 — 초기 3초 표시 후
  숨김, 스크롤 시 재표시, 멈춤 2.5초 후 다시 숨김 (호버·입력 중 유지)

### 변경

- 디자인 핸드오프 기반 UI 전면 리디자인 ("플랫 미니멀") — 브랜드
  컬러 토큰 교체(라이트/다크), 헤더 로고 락업 + 아이콘 전용 컨트롤,
  파일 목록 상태 점·드롭존 빈 상태 카드·2px 진행바, 미리보기 하단
  중앙 플로팅 컨트롤, 결과 패널 모드 필 세그먼트·dirty 상태 푸터
  통합, Markdown 표 플랫 스타일, 도움말 모달·드롭 오버레이 리스킨
- 클릭 가능한 모든 버튼에 포인터 커서 적용

## [0.4.2] - 2026-07-23

### 수정

- GUI에서 폴더를 드래그 앤 드롭하면 인식하지 못하던 문제 — 폴더 열기와
  동일하게 하위 폴더까지 재귀 스캔(깊이 8·최대 500파일)해 지원 문서를
  추가하도록 수정. 드롭 오버레이 안내 문구에 폴더 지원 명시

## [0.4.1] - 2026-07-23

### 수정

- HWPX 한컴 보조 평면 PUA-A(U+F0000~) 기호가 깨져 보이던 문제 — 한/글이
  Plane 15 사설영역에 저장하는 기호(삼각형 불릿 ▶◀▲▼, 수식 괄호 조각,
  음표 등) 57건을 표준 유니코드로 정규화 (함초롬바탕 글리프 비트맵 대조로
  자동 생성 + 시각 검증). 매핑에 없는 코드는 원본 유지

## [0.4.0] - 2026-07-23

### 추가

- **HTML → Markdown 변환** — 지원 포맷에 HTML 추가
  - 로컬 `.html`/`.htm` 파일 및 http(s) URL 입력 지원
  - `@mozilla/readability` + `linkedom` 기반 본문 추출 (nav·사이드바·푸터·광고
    제거, 실패 시 전체 폴백, `--no-extract`로 비활성)
  - SPA 렌더링 (`--render`): 시스템 Chrome으로 JS 렌더링 후 DOM 캡처
    (`playwright-core` optionalDependency, `--wait-selector`/`--timeout` 지원)
  - 원격 이미지 다운로드 (`--download-images`): `{문서명}_images/`에 저장,
    실패 시 원본 URL 유지, 문서당 50개 한도
  - SSRF 방어: `safeFetch`를 core로 승격해 URL fetch·이미지 다운로드·SPA
    sub-resource 요청까지 사설 대역 차단
- **블로그 플랫폼 대응** — 네이버 블로그 등 iframe 껍데기 페이지의 본문
  프레임 자동 추적, 본문 컨테이너 힌트 셀렉터(SmartEditor·티스토리·
  `itemprop=articleBody`), lazy-load 이미지 원본 승격, zero-width 문자 제거
- **앱: URL 변환 + 원본 미리보기** — 파일 목록에 URL 입력(🔗), 변환 전
  "추출된 본문 미리보기" 표시 (사이드카 `--html` + DOMPurify)
- **앱: 파일 탐색기·폴더 열기** — 네이티브 다이얼로그로 파일 다중 선택,
  폴더 선택 시 하위 폴더를 재귀 스캔해 트리로 표시 (기본 접힘, 전체
  펼침/닫힘 토글, 폴더 체크박스로 하위 전체 일괄 선택)

### 수정

- HWPX 체크박스 등 PUA 심볼 문자(U+F0xx)가 보이지 않던 버그 — Wingdings
  계열 12개 코드를 유니코드(■ □ ☑ ✓ 등)로 정규화
- 배포 번들 빌드가 `playwright-core` 포함 시도로 실패하던 문제
- CLI `--version`이 구버전(v0.1.0)을 출력하던 문제

### 보안

- pnpm audit 취약점 34건 해결 (`undici`, `vite`, `hono`, `dompurify`,
  `esbuild`, `fast-uri` 등 — `pnpm.overrides`로 패치)
- HTML sanitize: 제어문자 난독화 `javascript:` URI 우회 차단, `data:`는
  이미지만 허용

## [0.3.0] - 2026-05-15

### 추가

- **Cmd/Ctrl+F 통합 검색** — 모든 편집/뷰어 영역에서 일관된 단축키
  - Markdown 소스 편집기 (CodeMirror): 매치 하이라이트 + 다음/이전 이동
  - WYSIWYG 미리보기 (Milkdown) 및 미리보기 패널: DOM 텍스트 검색
  - DOCX 원본 뷰어: HTML 텍스트 검색 + 자동 스크롤
  - HWPX 원본 뷰어: rhwp 문서 트리(본문 + 표 셀 + 중첩 표 셀)를 직접 순회한
    텍스트 인덱스 기반. SVG 좌표로 하이라이트 오버레이
  - 검색바는 상단 고정, 재오픈 시 자동 포커스

- **Markdown 파일 직접 등록** — `.md` 파일 D&D 시 변환 큐를 거치지 않고
  즉시 자동 로드 (변환 skip + 원본 미리보기 placeholder)

### 수정

- HWPX 표 안 표(table-in-cell) 변환 누락 — 셀 내부 중첩 `<hp:tbl>`을
  인라인 평탄화 (`(표 R×C) 행1셀1 | 행1셀2 / ...` 형식, 깊이 5단)
- HWPX 표 colSpan/rowSpan 병합 셀 누락 — grid normalize로 빈 셀 padding
- 소스 편집기 매 키 입력마다 재마운트되어 입력 누락·undo 실패하던 문제
- `.md` 자동 로드가 spinner에서 멈추던 버그
- HWPX 뷰어에서 Cmd+F 검색바 미표출 / 스크롤 시 사라짐 / 재오픈 포커스 안 감

### 보안

- pnpm audit 취약점 4건 해결 (`fast-uri`, `fast-xml-builder`, `hono`,
  `ip-address` — 모두 transitive 의존, `pnpm.overrides`로 패치)

## [0.2.0] - 2026-04-17

### 추가

- **DOC (레거시 Word) 지원** (Phase 8)
  - `.doc` (Word 97-2003) 파일을 DOCX로 선변환 후 Markdown으로 변환
  - LibreOffice headless를 1순위 변환 도구로 사용 (크로스플랫폼, 이미지 보존)
  - macOS에서 LibreOffice 미설치 시 textutil fallback (텍스트만 변환)
  - `PAPER_MD_STUDIO_LIBREOFFICE` 환경변수로 커스텀 경로 지정 가능
  - App에서 `.doc` 파일 드래그 앤 드롭 + 뷰어 + 배치 변환 지원

## [0.1.0] - 2026-04-16

첫 번째 릴리스. HWP/HWPX/DOCX/PDF를 Markdown으로 변환하고 편집하는 데스크톱 앱.

### 추가

- **문서 변환** (Phase 1~2)
  - HWPX → Markdown (표, 스타일, 이미지 추출 포함)
  - DOCX → Markdown (mammoth + turndown)
  - PDF → Markdown (텍스트 추출)
  - HWP 5.0 바이너리 → HWPX → Markdown (Java hwp2hwpx 경유)
  - 이미지 추출: `{문서명}_images/` 디렉토리에 저장, 상대경로 참조
  - HWPX 볼드/이탈릭/취소선 스타일 변환

- **CLI** (Phase 1)
  - `paper-md-studio <파일>` 커맨드라인 변환
  - `--output`, `--images-dir`, `--html` 옵션
  - 한글 파일명 NFD→NFC 정규화 (macOS 대응)

- **Tauri 데스크톱 앱** (Phase 3~7)
  - 드래그 앤 드롭 파일 등록
  - 3패널 레이아웃: 파일 목록 / 원본 뷰어 / 변환 결과
  - 원본 뷰어: PDF (Canvas), DOCX/HWPX (HTML 렌더링)
  - 4-모드 에디터: 보기 / 편집(WYSIWYG) / 소스(CodeMirror) / 분할
  - `Cmd/Ctrl+S` 덮어쓰기, `Cmd/Ctrl+Shift+S` 다른 이름으로 저장
  - `Cmd/Ctrl+Shift+F` 결과 패널 전체화면 토글
  - 시스템/라이트/다크 테마 전환 (localStorage 영속화)
  - 배치 변환: 다중 파일 동시성 5 병렬 처리
  - 출력 디렉토리 선택 (설정 영속화)
  - 실패 파일 개별/일괄 재시도
  - 멀티 셀렉트 (체크박스 + Cmd/Shift 모디파이어)
  - 파일 충돌 프롬프트 (덮어쓰기/다른 이름/취소)
  - 빈 테이블 행 정리 + 1-step 취소

- **패키징** (Phase 7)
  - macOS Apple Silicon (.dmg) + Windows x64 (.msi) 배포
  - 번들 Node 런타임 (v20.18.0)
  - 번들 최소 JRE (jlink, HWP 변환용)
  - GitHub Actions CI/Release 파이프라인

### 알려진 제한사항

- PDF 이미지 추출 미지원
- DOC (레거시 Word) 미지원 (v2 예정)
- macOS: Apple Developer 인증서 미서명 (ad-hoc), Gatekeeper 우회 필요
- Windows: WebView2 런타임 자동 다운로드 (첫 실행 시)
