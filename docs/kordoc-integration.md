...
**잠정 권고**: PDF 를 kordoc 으로 전환하되 표 감지 재시도 전략을 얹는다.
단, 어절 공백 소실(`제21조및제3항의`, `Ⅰ.사업개요`)이 kordoc 쪽 실측 약점이므로
**실무 문서(특히 한컴 출력 PDF) 확보 후 재실측 전까지 확정하지 말 것.**
