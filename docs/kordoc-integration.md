...
- **다음에 할 일 (우선)**: **CI 복구** — `pnpm audit` 단계가 빨갛다.
  kordoc 도입 이전부터 실패하던 문제이며, 처리 방안은
  [`docs/security-audit-remediation.md`](./security-audit-remediation.md) 에
  전부 정리해두었다(취약점 20건 인벤토리·override 갱신 목록·검증 절차).
- **그 다음**: §5 "추가 표본 확보" 의 빈칸(한컴 네이티브 PDF·2단 조판·
  복잡 병합표)을 채워 재실측 → K2 확정. 그 뒤 K3(Java 툴체인 대체) 검토.
