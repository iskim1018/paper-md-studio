...
**W2. GFM 직렬화 후처리** ✅ **완료 (2026-08-09)** — `parsers/html-tables-to-gfm.ts`
신규. 결과: 토큰이 **4표본 전부 Java 이하**로 내려갔다 (+47.5%→-0.2%, +4.9%→-1.9%,
+56.8%→-1.7%, +14.7%→-1.8%). HTML 표 잔존 0, 두 경로의 병합 화살표 수가 거의
일치(표본3은 24/86 완전 일치). 속도 우위 10~20× 유지.

- **셀 병합 표기 도입** — GFM 표에는 병합 문법이 없어 종전 계약(빈 셀 padding)은
  "진짜 빈칸"과 "병합 자리"를 구분하지 못했다. 표본4에서 빈칸처럼 보이는 셀
  3,902개 중 **59%가 병합 자리**였다. 원점 방향 화살표로 표기한다 —
  `←` 왼쪽 셀과 병합, `↑` 위 셀과 병합. **토큰 비용 0** (빈 GFM 셀도 padding
  공백으로 이미 토큰 1개를 쓴다). 대안인 "내용 반복"은 표본4에서 +15,239토큰
  (+21.6%)이라 제품 목표와 충돌해 기각. 자체 HWPX 파서에도 같이 적용했다 —
  같은 표가 확장자에 따라 다르게 나오면 안 된다.
- **글리프 정규화 통일** — kordoc의 PUA 매핑을 U+F000~U+F0FF 전 대역 실측해
  합집합을 만들었다(우리 12개 + kordoc 48개 → **50개**). 겹치는 코드는 자체
  매핑 유지 — kordoc은 `0xF06D`를 `●`로 두지만 Wingdings 0x6D는 white circle이라
  우리가 맞다. kordoc이 이미 변환해 버린 글자는 코드포인트를 되살릴 수 없어
  글자 대 글자로 통일한다(`canonicalizeGlyphs`: `◻`→`□`, `✔`→`✓` 등 9쌍).
  이것도 양쪽 경로에 건다 — 한쪽에만 걸면 실제로 Java `✔` / kordoc `✓` 로 갈렸다.

> ⚠️ **계측기 결함 3연속** — W1에서 HTML 표 미집계를 고쳤는데, 이후 같은 부류를
> 두 번 더 밟았다. ① 비용을 정하는 건 "표 개수"가 아니라 "셀 개수"(표본3은 표
> 3개에 `<td>` 1,246개) ② 하니스의 PUA 정규식에 **리터럴 PUA 문자**를 넣었다가
> 그게 소실돼 `[-]`(하이픈)이 되는 바람에 GFM 구분선 `---`을 PUA로 세고 있었다
> (표본4의 "PUA잔존 1,870"은 전부 하이픈이었다. 실제 잔존은 **0**).
> → **보이지 않는 문자를 소스에 리터럴로 두지 말 것.** `pua-symbols.ts`의 맵
> 키도 이 때문에 코드포인트(`0xF0xx`)로 바꿨다.
