...
/** URL 사전 검증 옵션 (SPA 렌더링 등 fetch 를 직접 쓰지 않는 경로용) */
...
export async function safeFetch(
  ...
  for (let hop = 0; hop <= opts.maxRedirects; hop += 1) {
    const guard = await validateFetchUrl(currentUrl, {
...
/**
 * fetch 없이 URL 만 사전 검증한다 (스킴 + DNS 해석 후 사설 대역 차단).
 * SPA 렌더링처럼 브라우저가 직접 네비게이션하는 경로에서 사용.
 */
